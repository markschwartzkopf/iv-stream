...
nodecg.listenFor('addArrayItem', (arg) => {
    if (gameDefs[arg.game] &&
...
nodecg.listenFor('removeArrayItem', (arg) => {
    if (gameDefs[arg.game] &&
        gameDefs[arg.game].arrays[arg.array] &&
        gamesDataRep.value &&
        gamesDataRep.value[arg.game] &&
...
nodecg.listenFor('vfRenownChange', (arg) => {
    vfRenownChange(arg.hadria ? 'hadria' : arg.demiIndex, arg.newVal);
});
nodecg.listenFor('resetGame', (gameName) => {
...
function verifyGameData(game, checkData) {
    const gameData = {};
    ...
    for (let i = 0; i < arraySize; i++) {
        ...
        for (const [key, value] of Object.entries(arrayDef.fields)) {
            ...
            switch (value.type) {
                ...
                default:
                    nodecg.log.error(`code array field type "${value.type}" creation`);
                    break;
            }
...
function vfRenownChange(demigodIndex, newRenown, demigodData) {
    ...
    const demigods = data;
    if (newRenown > 12 || newRenown < 0)
        return;
    if (demigodIndex === 'hadria') {
        const hadria = demigods[0];
        if (hadria && hadria.renown)
            hadria.renown.val = newRenown;
    ...
    const oldRenown = demigod.renown ? demigod.renown.val : null;
    if (oldRenown === newRenown)
        return; //likely newly instatiated item
    demigod.renown = { val: newRenown, old: demigod.renown.old };
    if (oldRenown !== null) {
