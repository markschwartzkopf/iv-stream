...
function initArray(gameName: string, arrayName: string) {
	NodeCG.waitForReplicants(gamesdataRep, gamesRep).then(() => {
		...
		const repArrayData: GameArrayData[] = JSON.parse(
...
const vfSvgObject = document.getElementById(
	'vf-svg-object'
	...
	setTimeout(() => {
		...
		if (svgDoc) {
			Object.keys(demigodMarkers).forEach((key) => {
				...
				if (marker)
					demigodMarkers[key].marker = marker as any as SVGGraphicsElement;
			});
	...
	switch (arg.game) {
		case 'Veiled Fate': {
			...
			switch (arg.array) {
				case 'Hadria': {
					...
					if (hadria.marker) {
						...
						anim.setAttributeNS(
							null,
							'values',
							`${roundString(oldVal, 2)} 0; ${roundString(newVal, 2)} 0`
						);
									...
									if (
										...
										const moveAnim = () => {
											...
											anim.setAttributeNS(
												null,
												'values',
												`${roundString(oldVal, 2)} 0; ${roundString(
													newVal,
													2
												)} 0`
											);
											...
											anim.onend = () => {
												...
												if (needMarker)
													setTimeout(() => {
														marker.children[renown[j].smited ? 1 : 0].animate(
															demigodMarkerInKeyframes,
															{
										...
										if (needMarker) {
											const inAnim = marker.children[
												renown[j].smited ? 1 : 0
											].animate(demigodMarkerInKeyframes, {
												duration: MARKER_SHOW_DURATION,
												fill: 'forwards',
											});
											inAnim.onfinish = () => {
				...
				default: {
					nodecg.log.error(
						`No graphics for array "${arg.array}" in game "${arg.game}"`
					);
					break;
				}
			}
