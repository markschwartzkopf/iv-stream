...
nodecg.listenFor('addArrayItem', (arg: AddArrayItemArg) => {
	if (
		...
		gamesDataRep.value[arg.game][arg.array] = verifyGameArray(
			gameDefs[arg.game].arrays[arg.array],
			[...gamesDataRep.value[arg.game][arg.array], null]
		);
	} else {
		nodecg.log.error(
...
function verifyGamesData(gameDefs: GameDefs, checkData: null | GamesData) {
	...
	for (const [key, value] of Object.entries(gameDefs)) {
		gamesData[key] = verifyGameData(
			value,
			checkData && checkData[key] ? checkData[key] : null
		);
	}
	return gamesData;
}
...
function verifyGameArray(
	arrayDef: GameArrayDef,
	checkArray: null | (GameArrayData | null)[]
) {
	const gameArrayData: GameArrayData[] = [];
	const arraySize =
		checkArray && checkArray.length > arrayDef.min
			? checkArray.length
			: arrayDef.min;
	for (let i = 0; i < arraySize; i++) {
		...
		for (const [key, value] of Object.entries(arrayDef.fields)) {
			const existing =
				checkArray && checkArray[i] ? checkArray[i]![key] : undefined;
			switch (value.type) {
				case 'integer':
					if (
						existing &&
					...
					if (
						...
						while (
							gameArrayData.map((x) => x[key]).indexOf(arrayItem[key]) !== -1 &&
							(value.max === undefined ||
								(arrayItem[key].val as number) <= value.max)
						)
				...
				case 'string': {
					let proposedString =
						existing !== undefined && typeof existing.val === 'string'
							? existing.val
							: value.unique
							? '1'
							: '';
					...
					if (value.unique)
						while (
							gameArrayData.map((x) => x[key].val).indexOf(proposedString) !==
							-1
						)
				...
				case 'stringEnum': {
					let proposedVal =
						existing !== undefined && typeof existing.val === 'number'
							? existing.val
							: 0;
					if (
						value.unique &&
				...
				case 'VFRenown': {
					...
					if (existing === undefined && !value.hadria)
						vfRenownChange(arrayItem, renown, gameArrayData);
					break;
				}
				default:
					nodecg.log.error(
						`code array field type "${(value as any).type}" creation`
					);
					break;
			}
...
function vfRenownChange(
	...
	const demigod =
		typeof demigodIndex === 'object' ? demigodIndex : demigods[demigodIndex];
	const sameRenownItems = demigods.filter(
		(x) => x.renown && x.renown.val === newRenown
	);
	...
	if (oldRenown !== null) {
		const sameRenownItems = demigods.filter(
			(x) => x.renown && x.renown.val === oldRenown
		);
		sameRenownItems.sort((a, b) => (a.place.val > b.place.val ? 1 : -1));
		for (let i = 0; i < sameRenownItems.length; i++) {
...
function vfSmite(index: number) {
	...
	const sameRenownItems = demigods.filter(
		(x) => x.renown && x.renown.val === demigod.renown.val
	);
