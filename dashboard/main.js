...
const activeGameRep = nodecg.Replicant('active-game');
const gameSelectorDiv = document.getElementById('game-selector');
const arrayHolder = document.getElementById('array-holder');
        ...
        if (activeGameRep.value && gamesdataRep.value && gamesRep.value) {
            ...
            for (const [arrayName, arrayData] of dataArrays) {
                ...
                for (let i = 0; i < arrayData.length; i++) {
                    ...
                    const row = verifyElement(arrayTable, i + headerElements, arrayDef.display === 'table' ? 'tr' : 'div', i === arrayData.length - 1);
                    for (let j = 0; j < arrayFields.length; j++) {
                        const field = arrayFields[j][0];
                        const fieldDef = arrayFields[j][1];
                        const fieldCell = verifyElement(row, j, arrayDef.display === 'table' ? 'td' : 'div', j === arrayFields.length - 1);
                        switch (fieldDef.type) {
                            ...
                            case 'integer': {
                                ...
                                if (changed) {
                                    ...
                                    fieldCell.classList.remove('changed');
                ...
                if (arrayData.length < arrayDef.max) {
                    ...
                    plus.onclick = () => {
                        const arg = { game: activeGame, array: arrayName };
...
function verifyElement(parent, index, nodeName, last) {
...
function verifyIncDecElement(minusFunc, plusFunc, val, parent, index, changed) {
...
function submitArray(gameName, arrayName, noChange) {
    NodeCG.waitForReplicants(gamesdataRep, gamesRep).then(() => {
        var _a, _b;
        if (!gamesdataRep.value || !gamesRep.value)
            return;
        const newRepArray = JSON.parse(JSON.stringify(gamesdataRep.value[gameName][arrayName]));
        const animateData = [];
        ...
        if (!noChange)
            gamesdataRep.value[gameName][arrayName] = newRepArray;
    });
}
